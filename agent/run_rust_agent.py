    ...
    if thinking_capture is not None:
        try:
            ...
            for _module_name in _modules_seen:
                ...
                # Best-effort cumulative diff (base_commit → HEAD) via the shared
                # module_file_patch with rust's target/ filter. Overcounts co-modified
                # files vs a per-module pre/post window but preserves scoring data.
                try:
                    _module_patch_bs = module_file_patch(
                        local_repo, example["base_commit"], "HEAD", target_edit_files,
                        filter_fn=_filter_rust_patch_lenient, logger=logger,
                    )
                except Exception:
                    _module_patch_bs = ""
