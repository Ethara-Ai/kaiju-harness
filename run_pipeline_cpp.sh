#!/usr/bin/env bash
# ============================================================
# 3-Stage SDE-I Trajectory Pipeline for Commit0 — C++
# ============================================================
#
# Usage:
#     bash run_pipeline_cpp.sh --model <preset|model_id> --dataset <name>
#
# Examples:
#     bash run_pipeline_cpp.sh --model opus --dataset fmt_cpp
#     bash run_pipeline_cpp.sh --model kimi --dataset fmt_cpp
#     bash run_pipeline_cpp.sh --model opus --dataset ./fmt_cpp_dataset.json
#     bash run_pipeline_cpp.sh --model opus --dataset fmt_cpp --num-samples 3
#     nohup bash run_pipeline_cpp.sh --model opus --dataset fmt_cpp > logs/opus_fmt_cpp.log 2>&1 &
#
# Requirements: jq, bc
# ============================================================
...
# F8 audit fix: C++ compilation (clang++) is comparable to Java (javac) — both
...
print_usage() {
    cat <<'USAGE'
Usage: run_pipeline_cpp.sh --model <preset|model_id> --dataset <name> [OPTIONS]
...
Dataset examples:
  ...
  abseil_cpp           Uses abseil_cpp_dataset.json, repo_split=abseil_cpp
...
Options:
  ...
  --inactivity-timeout <s>   Kill agent if no log activity for N seconds (default: 1200)
...
resolve_dataset() {
    ...
    if [[ "$arg" == *.json ]] || [[ "$arg" == */* ]]; then
        ...
        if [[ -n "$REPO_SPLIT_OVERRIDE" ]]; then
            ...
            # Strip _cpp_dataset first so "fmt_cpp_dataset.json" → "fmt"
            # (matches the repo name "fmt" in zahgon/fmt), not "fmt_cpp".
            local basename
            basename=$(basename "$arg" .json)
            basename="${basename%_cpp_dataset}"
            basename="${basename%_dataset}"
            REPO_SPLIT="$basename"
        fi
        DATASET_SHORT=$(basename "$arg" .json)
        DATASET_SHORT="${DATASET_SHORT%_cpp_dataset}"
        DATASET_SHORT="${DATASET_SHORT%_dataset}"
        return
    fi
    ...
    if [[ -f "$candidate" ]]; then
        ...
        # Strip _cpp suffix from named arg too: "fmt_cpp" → "fmt"
        local arg_stripped="${arg%_cpp}"
        REPO_SPLIT="${REPO_SPLIT_OVERRIDE:-$arg_stripped}"
        DATASET_SHORT="${arg_stripped}"
        return
    fi

    # Case 3: named split from commit0 C++ constants
    # NOTE: No C++-specific HuggingFace dataset exists yet.
    # CPP_SPLIT keys are recognised so we can give a helpful error,
...
for k in sorted(CPP_SPLIT.keys()):
    print(k)
" 2>/dev/null || true)

    if echo "$known_splits" | grep -qx "$arg"; then
        echo "Error: Split '$arg' is a known C++ split, but no C++ HuggingFace dataset exists."
        echo "Please provide a local dataset JSON file instead."
        echo ""
        echo "Example: bash run_pipeline_cpp.sh --model opus --dataset ./fmt_cpp_dataset.json --repo-split $arg"
        exit 1
    fi
    ...
    echo "  - A commit0 C++ split name"
    echo ""
    echo "Available local datasets:"
    for f in "${BASE_DIR}"/*_cpp_dataset.json "${BASE_DIR}"/*_dataset.json; do
        [[ -f "$f" ]] && echo "  $(basename "${f}" _dataset.json)"
    done
    echo ""
    echo "Available C++ splits:"
...
BASE_BRANCH_NAME="${BRANCH_OVERRIDE:-aider-cpp-${MODEL_SHORT}-${DATASET_SHORT}}"
...
BASE_RUN_ID_FLAT=$(echo "cpp_${MODEL_SHORT}_${DATASET_SHORT}" | tr -dc 'a-zA-Z0-9._-')
...
set_sample_vars() {
    ...
    if is_consolidated; then
        ...
        LOG_BASE="${BASE_DIR}/logs/agent/cpp/${DATASET_DIR_NAME}/${MODEL_DIR_NAME}/run_${sample_idx}"
...
write_agent_config() {
  ...
  throw std::runtime_error("STUB: not implemented") markers, return {} for
  constexpr stubs, or std::abort() for noexcept stubs) and pass the unit tests.

  Do not change function signatures, class definitions, or virtual function declarations,
  as they may be referenced from other code like unit tests.

  Do not modify CMakeLists.txt, meson.build, Makefile, or configure.ac, or any test
  files. Do not use inline assembly or compiler-specific intrinsics unless the existing
...
run_agent() {
    ...
    local cmd=(
        "$VENV_PYTHON" -m agent.cli_cpp "$branch"
        --backend "$BACKEND"
...
parse_eval_output() {
    ...
    # Aggregate across all "repo,runtime,passed/total,status" lines
    ...
    while IFS= read -r line; do
        ...
        if [[ "$line" == *","*"/"* ]]; then
            ...
            # PATCH_APPLY_FAILED / OUTPUT_MISSING / CHEAT_DETECTED otherwise). Any
            # non-TESTS_RAN status means the 0/N is a build/patch/infra/forgery
            # failure, NOT a genuine 0% model score — surface it (mirrors C/rust/go).
...
print_summary_table() {
    log ""
    log "=========================================================================================="
    log "RESULTS SUMMARY — SDE-I 3-Stage C++ Pipeline"
...
run_single_sample() {
    ...
    log "======================================================================"
    log "Commit0 SDE-I 3-Stage C++ Pipeline"
...
print_pass_at_k_summary() {
    ...
    log ""
    log "=========================================================================================="
    log "PASS@${k} SUMMARY — ${MODEL_SHORT} / ${DATASET_SHORT} (C++)"
    log "=========================================================================================="
    log ""
...
main() {
    ...
    if [[ "$SAMPLES_COMPLETED" -eq "$NUM_SAMPLES" ]]; then
        log "C++ pipeline complete. All ${NUM_SAMPLES} sample(s) succeeded."
        PIPELINE_SUCCESS="true"
    elif [[ "$SAMPLES_COMPLETED" -gt 0 ]]; then
        log "C++ pipeline complete. ${SAMPLES_COMPLETED}/${NUM_SAMPLES} sample(s) succeeded."
        PIPELINE_SUCCESS="true"
    else
        log "C++ pipeline FAILED. No samples completed successfully."
    fi
}
