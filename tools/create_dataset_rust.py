"""Create a HuggingFace dataset from prepared Rust repo entries.

B2: mirrors create_dataset_go.py so every language has a `create_dataset_<lang>`
tool. Previously Rust ran the prepare stage and wrote entries.json but had no
validation/consolidated-staging step, so downstream dataset packaging had to be
done ad-hoc.

Takes output of prepare_repo_rust.py (dataset_entries.json) and:
1. Validates entries match the RustRepoInstance schema
2. Adds entries to RUST_SPLIT constants
3. Uploads to HuggingFace (optional)
4. Generates commit0 config files (.commit0.rust.yaml)

Usage:
    python -m tools.create_dataset_rust dataset_entries.json --output custom_dataset.json
    python -m tools.create_dataset_rust dataset_entries.json --upload --hf-repo Ethara-Ai/commit0_rust
    python -m tools.create_dataset_rust dataset_entries.json --patch-constants
"""
...
# B2: same shape guard as create_dataset.py:75 (T8). Rust commits are SHA-1
# (40 hex) at rest, but we accept 7-64 to allow abbreviated inputs.
_COMMIT_SHA_RE = re.compile(r"^[0-9a-f]{7,64}$")


def validate_entry(entry: dict, index: int) -> list[str]:
    """Validate a single Rust dataset entry. Returns list of issues."""
    issues: list[str] = []
    ...
    if entry.get("language") != "rust":
        issues.append(f"[{index}] language must be 'rust', got '{entry.get('language')}'")
    ...
    for sha_field in ("base_commit", "reference_commit"):
        val = entry.get(sha_field, "")
        if sha_field in entry:
            if len(val) < 7:
                issues.append(f"[{index}] {sha_field} too short: {val!r}")
            elif not _COMMIT_SHA_RE.match(val):
                issues.append(
                    f"[{index}] {sha_field} is not a valid hex git SHA: {val!r}"
                )
...
def generate_rust_split_constants(
    entries: list[dict], split_name: str = "custom_rust"
) -> str:
    repo_names = sorted(entry["repo"] for entry in entries)

    lines = [
        f"# Rust split: {split_name} ({len(entries)} repos)",
        f'RUST_SPLIT["{split_name}"] = {{',
    ]
    ...
    lines.append("")
    lines.append("# Individual Rust repo splits")
    for name in repo_names:
        short = name.split("/")[-1]
        lines.append(f'RUST_SPLIT["{short}_rust"] = ["{short}"]')
...
def create_hf_dataset_dict(entries: list[dict]) -> list[dict]:
    ...
    Assigns a fresh UUID4 to each entry's ``id`` field so each dataset build
    represents a distinct experiment instance.
    """
...
def generate_commit0_rust_yaml(
    entries: list[dict], split_name: str, dataset_name: str
) -> str:
    repo_names = sorted(entry["repo"] for entry in entries)

    yaml_content = f"""# commit0 Rust config for dataset: {split_name}
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create HuggingFace dataset from prepared Rust entries"
    )
    parser.add_argument(
        "entries_file",
        help="Input dataset_entries.json from prepare_repo_rust.py",
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="custom",
        help="Name for the RUST_SPLIT constant (default: custom)",
    )
    parser.add_argument(
        "--upload",
        action="store_true",
    ...
    parser.add_argument(
        "--hf-repo",
        type=str,
        default="Ethara-Ai/commit0_rust",
        help="HuggingFace repo ID for upload (default: Ethara-Ai/commit0_rust)",
    )
    parser.add_argument(
        "--hf-token",
        type=str,
    ...
    parser.add_argument(
        "--patch-constants",
        action="store_true",
        help="Generate Python code to add to constants_rust.py",
    )
    parser.add_argument(
        "--generate-yaml",
        action="store_true",
        help="Generate .commit0.rust.yaml for the custom dataset",
    )
    parser.add_argument(
        "--outputs-root",
        type=str,
    ...
    if _consolidated and hf_entries and hf_entries[0].get("id"):
        try:
            ...
            for e in hf_entries:
                _cii(hf_entries[0]["id"], e["repo"].split("/")[-1], test_ids_subdir="rust_test_ids", repo_base="repos/rust")
        except Exception as _e:
            logger.warning("copy_inference_inputs failed: %s", _e)

    print(f"\n{'=' * 80}")
    print(f"RUST DATASET: {len(valid)} entries")
    print(f"{'=' * 80}")
    ...
    if args.patch_constants:
        constants_code = generate_rust_split_constants(valid, args.split_name)
        constants_file = Path(f"rust_split_{args.split_name}.py")
        constants_file.write_text(constants_code)
        logger.info("RUST_SPLIT constants written to %s", constants_file)
        print(f"\n# Add to commit0/harness/constants_rust.py:\n{constants_code}\n")

    if args.generate_yaml:
        yaml_content = generate_commit0_rust_yaml(valid, args.split_name, args.hf_repo)
        yaml_file = Path(f".commit0.rust.{args.split_name}.yaml")
