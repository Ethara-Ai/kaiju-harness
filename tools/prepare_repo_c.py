...
def _scrape_and_commit_spec(
    ...
    """Scrape a PDF spec and commit it into the current branch.

    Mirrors the Python ``prepare_repo.py`` spec step: scrape ``spec_url`` to a
    bz2-compressed PDF, copy it into the repo as ``spec.pdf.bz2``, commit it so
    it becomes part of the stubbed ``base_commit``. Best-effort: any failure
    (missing optional deps, blocked domain, scrape error) logs a warning and
    returns ``None`` so the caller keeps the original ``base_commit``.
    ...
    try:
        ...
        logger.warning(
            "  Spec scrape unavailable (%s) — install scrape deps; skipping",
            exc,
        )
        return None
    except Exception as exc:  # noqa: BLE001 - best-effort, never fatal
        logger.warning("  Spec scrape failed for %s: %s", repo_name, exc)
        return None
...
def prepare_one(
    ...
    # Optionally scrape a spec PDF and fold it into base_commit (best-effort).
    if scrape_spec_flag and spec_url:
        new_base = _scrape_and_commit_spec(
            repo_path, slug.split("/")[-1], spec_url, specs_dir
        )
        if new_base:
            base_commit = new_base
    elif scrape_spec_flag and not spec_url:
        logger.warning(
            "%s: --scrape-spec set but no --spec-url provided — skipping spec",
            slug,
        )
    ...
    if push and fork_org:
        try:
            subprocess.run(
                ["gh", "repo", "fork", slug, "--clone=false", "--org", fork_org],
                check=False,
                capture_output=True,
            )
            git(repo_path, "remote", "set-url", "origin",
                f"https://github.com/{target_repo_slug}.git")
            git(repo_path, "push", "-u", "origin", branch, "--force-with-lease")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="GitHub org to fork into (omit to keep entries pointing at the original)",
    )
    parser.add_argument(
        "--push",
        action="store_true",
    ...
    parser.add_argument(
        "--scrape-spec",
        action="store_true",
        help="Scrape a PDF spec from --spec-url and commit it into base_commit",
    )
    parser.add_argument(
        "--spec-url",
        type=str,
    ...
    for slug in candidates:
        try:
            entry = prepare_one(
                ...
                scrape_spec_flag=args.scrape_spec,
